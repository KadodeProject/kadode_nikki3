def get_importantWords(affiliation):
    ...
    for value in affiliation.values():  # 辞書型なのでvalues必要
        ...
        if (
            value["lemma"] == "さん"
            or value["lemma"] == "くん"
            or value["form"] == "Person"
            or value["form"] == "Name_Other"
        ):
            continue
        else:
            words[value["lemma"]] = (
                words[value["lemma"]] + 1 if value["lemma"] in words else 1
            )

    # それを登録する形に改変
    important_words_raw = []
    for key, value in words.items():
        important_words_raw.append({"name": key, "count": value})

    # 多い順に並び変え
    important_words = sorted(
        important_words_raw, key=lambda x: x["count"], reverse=True
    )  # タプル型になる
