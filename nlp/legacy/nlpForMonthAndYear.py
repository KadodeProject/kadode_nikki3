...
def nlpForMonthAndYear(user_id):
    ...
    for yMonthDate, yMonth_dic in yMonth_dicList.items():
        ...
        形容詞
        """
        adjective_rank_raw = collections.Counter(
            yMonth_dic["adjective_rank"]
        )  # 単語要素別にカウント
        adjective_rank_all = sorted(
            adjective_rank_raw.items(), key=lambda x: x[1], reverse=True
        )  # 値の大きい順にソート
        ...
        important_words
        """
        important_words_raw = collections.Counter(
            yMonth_dic["important_words"]
        )  # 単語要素別にカウント
        important_words_all = sorted(
            important_words_raw.items(), key=lambda x: x[1], reverse=True
        )  # 値の大きい順にソート
        ...
        special_people
        """
        special_people_raw = collections.Counter(
            yMonth_dic["special_people"]
        )  # 単語要素別にカウント
        special_people_all = sorted(
            special_people_raw.items(), key=lambda x: x[1], reverse=True
        )  # 値の大きい順にソート
        ...
        classification
        """
        classification_raw = collections.Counter(
            yMonth_dic["classifications"]
        )  # 単語要素別にカウント
        classification_all = sorted(
            classification_raw.items(), key=lambda x: x[1], reverse=True
        )  # 値の大きい順にソート
        ...
        db.set_depDate_json_data(
            "statistic_per_months",
            user_id,
            dateForDB,
            emotions=yMonth_dic["emotions"],
            word_counts=yMonth_dic["word_counts"],
            noun_rank=yMonth_dic["noun_rank"],
            adjective_rank=yMonth_dic["adjective_rank"],
            important_words=yMonth_dic["important_words"],
            special_people=yMonth_dic["special_people"],
            classifications=yMonth_dic["classifications"],
        )
        # createdatもupdatedadも自動で入らない
        now_jst = dt.now(JST)
        db.set_depDate_normal_data(
            "statistic_per_months",
            user_id,
            dateForDB,
            created_at=now_jst,
            updated_at=now_jst,
            statistic_progress=100,
        )
    ...
    for yearDate, year_dic in year_dicList.items():
        ...
        db.set_depDate_json_data(
            ...
            user_id,
            dateForDB,
            emotions=year_dic["emotions"],
            word_counts=year_dic["word_counts"],
            noun_rank=year_dic["noun_rank"],
            adjective_rank=year_dic["adjective_rank"],
            important_words=year_dic["important_words"],
            special_people=year_dic["special_people"],
            classifications=year_dic["classifications"],
        )
        # createdatもupdatedadも自動で入らない
        now_jst = dt.now(JST)
        db.set_depDate_normal_data(
