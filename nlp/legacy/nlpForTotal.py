...
def nlpForTotal(user_id):
    ...
    # 多い順に並び替え
    total_classifications = sorted(
        total_classifications.items(), key=lambda x: x[1], reverse=True
    )
    total_important_words = sorted(
        total_important_words.items(), key=lambda x: x[1], reverse=True
    )
    ...
    db.set_statistics_json(
        ...
        month_words=total_month_words,
        month_diaries=total_month_diaries,
        emotions=total_emotions,
        classifications=total_classifications,
        special_people=total_special_people,
        total_noun_asc=total_noun_asc,
        total_adjective_asc=total_adjective_asc,
    )
    db.set_statistics_value(
        user_id, total_words=total_words, total_diaries=total_diaries
    )
