...
class GetSameDayDiariesByDate
{
    ...
    {
        $diaries = Diary::with(['statisticPerDate', 'diaryProcessed'])->whereMonth('date', $date->month)->whereDay('date', $date->day)->where('date', '!=', $date)->orderby('date', 'desc')->get(['id', 'date', 'title', 'content', 'updated_at']);
